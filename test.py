...
class PromptIRModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = LinearWarmupCosineAnnealingLR(optimizer=optimizer,warmup_epochs=3,max_epochs=30)
...
if __name__ == '__main__':
    ...
    if testopt.mode == 0:
        ...
        desnow_base_path = testopt.denoise_path.replace('denoise', 'desnow')
        testopt.denoise_path = os.path.join(desnow_base_path, "degraded")
        desnow_set = DerainDehazeDataset(testopt, addnoise=False, sigma=15)
        test_Derain_Dehaze(net, desnow_set, task="desnow")
    elif testopt.mode == 4:
        print('Start testing snow removal...')
        desnow_base_path = testopt.denoise_path.replace('denoise', 'desnow')
        testopt.denoise_path = os.path.join(desnow_base_path, "degraded")
        desnow_set = DerainDehazeDataset(testopt, addnoise=False, sigma=15)
        test_Derain_Dehaze(net, desnow_set, task="desnow")
